...
- [ ] Linux 开发机的具体选型（D-017）：内核版本是否支持所需 io_uring 操作码；能否加载 `rdma_rxe`（Soft-RoCE 副线）；nsys / perf 权限。
...
- [x] 阶段 1 的月度里程碑 → 已回答：2026-09-23，按章排序与学习融合见 `09-learning-roadmap.md`。
- [ ] 实习投递时间点（按 `01` §5「三四个月内有可见贡献就去投」对齐到 `09` 的阶段 1 章节进度）。
